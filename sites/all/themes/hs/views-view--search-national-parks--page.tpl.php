...
<?php if(arg(0)=="search-national-parks" && count($view->result) == 0 && (!strpos($_SERVER['REQUEST_URI'],"?")))
{
?>
  <?php if ($exposed): ?>
...
/*
$_SESSION['np'][$md_var] = $arr;
print_r($_SESSION['np'][$md_var]);
*/
$md_var = md5($string);
...
{
	$sql = "insert into paging_full_record (unique_url,session_id,timestamp,records,uid,ip_add,page,url) values ('$md_var','$session_id',$timestamp,'$rec',$uid,'$ip','np','$string')";
